...
Как бот читает ClickUp:

- `по мне` — берёт ClickUp user id из `CLICKUP_TELEGRAM_USER_MAP`;
- `по всем`, `для всех` — не ставит фильтр по ответственному;
- `для Фатхулло`, `у Натальи` — фильтрует по имени ответственного в ответе ClickUp;
- `открытые` — статусы `задачи`, `в процессе`, `бэклог`;
- `в процессе`, `бэклог`, `выполненные` — фильтр по одноимённому статусу;
- `прошлая неделя` — календарная неделя понедельник-воскресенье;
- `последняя неделя`, `за неделю`, `last 1 weeks` — последние 7 дней от текущего момента;
- `дедлайн/срок` — фильтр по `due_date`;
- `созданные`, `обновленные`, `закрытые/выполненные` — фильтры по `date_created`, `date_updated`, `date_done`;
- подзадачи включены всегда через `subtasks=true`;
- длинные списки отправляются несколькими сообщениями.
