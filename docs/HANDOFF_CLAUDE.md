...
- вести закреплённый список задач из личного чата с Фатхулло;
- сохранять записи звонков из отдельной Telegram-группы в папку Google Drive;
...
Контур готов в коде. Логика такая:

1. Бота добавляют в группу, куда скидываются записи звонков.
2. В этой группе отправляют `/whoami`.
3. `chat_id` группы добавляют в два env-поля:

```bash
TG_AGENT_ALLOWED_CHAT_IDS=...
...
Что бот делает с записью:

- сохраняет голосовые, аудио, видео и аудио/видео-документы;
- раскладывает файлы по датам;
- пишет журнал `tg_call_recordings.jsonl`;
- короткие записи расшифровывает текущим voice-контуром;
- после расшифровки делает управленческий разбор: итог, решения, задачи, риски и вопросы;
...
## 11. Отправка сообщений в группы

Бот может писать в подключённые группы, если:

- бот уже добавлен в группу;
- группа есть в разрешённых `TG_AGENT_ALLOWED_CHAT_IDS`;
- бот видел этот чат или в команде указан точный `chat_id`.

Формат команды:

```text
/send_group chat_id | Дайте статус по задаче
```

Также работает естественная формулировка:

```text
напиши в группу ОГ | Дайте статус по задаче
```

Если группа не найдена, бот покажет список известных чатов.

## 12. Закреп задач в личном чате

Если Фатхулло ставит задачи в личном чате с ботом, бот собирает открытые задачи из этого же личного чата в одно сообщение.
...
После первой новой задачи в личном чате бот сам создаёт или обновляет закреп. В группах автозакреп не создаётся.
...
задачи в процессе
задачи бэклог
закрытые задачи clickup за последнюю неделю по всем
закрытые задачи clickup за прошлую неделю по всем
```

Правила чтения фильтров:

- `по мне` использует `CLICKUP_TELEGRAM_USER_MAP`;
- `по всем` отключает фильтр по ответственному;
- имя человека после `для` / `у` фильтруется по assignee name;
- `открытые` = `задачи`, `в процессе`, `бэклог`;
- `выполненные/закрытые` считаются по `date_done`, чтобы совпадать с ClickUp UI `Date closed`;
- `прошлая неделя` = календарная неделя, `последняя неделя` / `last 1 weeks` = скользящие последние 7 дней;
- подзадачи включены через `subtasks=true`.
